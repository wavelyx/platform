...
const MintToken = ({ tokenName }: { tokenName: string }) => {
    ...
    const tokenAddress = watch("tokenAddress"); useEffect(() => {
        if (tokenAddress) {
        ...
        <FormProvider methods={methods} onSubmit={handleSubmit(processForm)}>
            <Grid container sx={{ justifyContent: 'center', alignItems: 'center', my: 5 }} spacing={3}>
                <Grid item xs={11} md={6}>
                    <TokenManager
                        title=' Mint Tokens (Increase Supply)'
                        subtitle='Create new tokens and increase your token&apos;s circulating supply. This is an advanced operation with potential risks that can dilute the value of existing tokens. Use with extreme caution and conduct thorough research before minting.'
                    ...
                    <LoadingButton sx={{ mt: 3 }} disabled={!hasMintAuthority} loading={isSubmitting || isCheckingMintAuthority} fullWidth size='large' variant='contained' onClick={handleSubmit(processForm)}>
                        Mint Your Token (0.1 SOL)

                    </LoadingButton>
                    {hasMintAuthority ? null : (
                        <div style={{ color: 'red', marginTop: 16 }}>
                            This token doesn't have a mint authority enabled and cannot be minted.
                        </div>
                    )}
